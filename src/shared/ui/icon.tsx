...
/**
 * A unified Icon component that supports both Lucide icons and custom brand SVGs.
 */
export const Icon = React.forwardRef<SVGSVGElement, IconProps>(
  ({ name, lucide: Lucide, size = 18, className, ...props }, ref) => {
    if (Lucide) {
      return <Lucide size={size} className={className} {...(props as any)} />;
    }

    if (name && ICON_MAP[name]) {
      const isStroke = name === 'instagram' || name === 'ai-sparkles';
...
export const MessengerIcon = ({ className }: { className?: string }) => (
  <svg
    ...
    height="12"
    fill="currentColor"
    className={cn("text-messenger shrink-0", className)}
  >
    <path d="M12 2C6.477 2 2 6.145 2 11.243c0 2.913 1.448 5.501 3.7 7.208V22l3.39-1.859c.92.256 1.895.395 2.91.395 5.523 0 10-4.146 10-9.243S17.523 2 12 2zm1.26 12.15l-2.48-2.65-4.83 2.65 5.3-5.63 2.53 2.7 4.73-2.7-5.25 5.63z" />
  </svg>
);
