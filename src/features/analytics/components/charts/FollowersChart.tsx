...
const followersInsightCache = new Map<
  string,
...
export function FollowersChart({
  ...
  useEffect(() => {
    ...
    async function loadAIInsight() {
      ...
      try {
        ...
        if (active) {
          if (res.content) {
            followersInsightCache.set(cacheKey, {
              content: res.content,
            ...
            if (
              ...
              (res.error.includes("rate_limit_exceeded") ||
                res.error.includes("Please try again in"))
            ) {
  ...
  if (isInstagramDynamics) {
    const hasValidAiRating =
      aiInsight && aiInsight.rating && RATING_CONFIG[aiInsight.rating];
          ...
          <div className="lg:col-span-8 flex flex-col gap-5 justify-between">
            ...
            <div className="grid grid-cols-3 divide-x divide-base-content/10 w-full">
              ...
              <div className="flex flex-col px-4 py-1">
                ...
                <span className="text-lg font-black text-error font-mono">
                  {totalUnfollows.toLocaleString()}
                </span>
              </div>
              <div className="flex flex-col pl-4 pr-0 py-1">
                ...
                <span
                  className={`text-lg font-black font-mono ${netGrowth >= 0 ? "text-success" : "text-error"}`}
                >
                  {netGrowth >= 0
            ...
            >
              {followsAndUnfollows.length === 0 ? (
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-base-content/20 text-sm font-medium">
                    Chưa có dữ liệu biến động cho khoảng thời gian này
                  </span>
                </div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart
                    data={followsAndUnfollows}
                        ...
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-base-300/95 backdrop-blur-xl border border-base-content/10 p-3 rounded-xl shadow-2xl min-w-[140px]">
                              <div className="text-2xs text-base-content/40 font-bold uppercase tracking-wider mb-2 font-mono">
                                {label}
                              </div>
                      ...
                      {rateLimitMessage && (
                        <div className="alert alert-warning text-xs py-2 px-3 rounded-lg border border-warning/20 bg-warning/5 flex items-center gap-2">
                          <Icon
                            ...
                            size={14}
                            className="text-warning shrink-0 animate-pulse"
                          />
                          <span className="font-semibold text-base-content leading-normal">
                            {rateLimitMessage.replace(
                              "Please try again in",
                              "Đạt giới hạn lượt gọi AI. Vui lòng thử lại sau",
                            )}
                          </span>
                        </div>
                      )}
                        ...
                        <div className="flex flex-col gap-3 text-sm leading-relaxed">
                          <div className="font-medium text-base-content/90 flex items-start gap-2">
                            <Icon
                              lucide={BarChart2}
                              size={14}
                              className="text-info shrink-0 mt-1.5"
                            />
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col gap-3 text-sm leading-relaxed">
                        <div className="text-base-content/90 flex items-start gap-2">
                          ...
                          />
                          <span>
                            Tài khoản ghi nhận {totalFollows.toLocaleString()}{" "}
                            lượt theo dõi mới và{" "}
                            {totalUnfollows.toLocaleString()} lượt bỏ theo dõi,
                            đem lại tăng trưởng ròng là{" "}
                            {netGrowth >= 0 ? "+" : ""}
                            {netGrowth.toLocaleString()} (tỷ lệ{" "}
                            {ratio.toFixed(2)}x) trong {range}.
                          </span>
                        </div>
                        <div className="bg-background/80 p-2 rounded-md text-base-content/80 font-medium flex items-start gap-2">
                          <Icon
                          ...
                          />
                          <span>
                            Kỳ vọng: Duy trì tăng trưởng dương ổn định, tăng
                            thêm 500-1000 followers trong 30 ngày tới.
                          </span>
                        </div>
                      </div>
                    </div>
  ...
  return (
    <div className="w-full bg-base-100 border border-base-content/5 shadow-sm rounded-2xl p-6 flex flex-col gap-6 transition-all duration-300 hover:shadow-md">
      ...
      <div
        ...
        className="relative mt-2 text-base-content/70"
      >
        <ResponsiveContainer width="100%" height="100%">
