  ...
  return (
    ...
    >
        ...
        <div className="w-20 h-10 opacity-30 group-hover:opacity-80 transition-opacity">
          {sparklineData.length > 0 && (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sparklineData.map((v, i) => ({ v, i }))} margin={{ top: 2, bottom: 2 }}>
      ...
      <div className="flex flex-col gap-1">
        <span className="text-xs font-bold text-base-content/40 uppercase tracking-widest font-mono">{label}</span>
        ...
        <div className="flex items-baseline justify-between gap-2 mt-1">
          ...
          <div className="flex items-center gap-1.5">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5 font-mono ${
              isPositive ? 'text-success bg-success/10 border border-success/5' : 
              trend === '—' ? 'text-base-content/40 bg-base-200 border border-base-content/5' : 'text-error bg-error/10 border border-error/5'
            }`}>
              {trend !== '—' && (isPositive ? '▲' : '▼')} {trend.replace('+', '').replace('-', '')}
            </span>
            {delta !== undefined && delta !== 0 && (
              <span className="text-[10px] font-bold text-base-content/30 font-mono">
