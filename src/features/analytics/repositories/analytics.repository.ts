...
export async function getAnalyticsForPeriod(filter: AnalyticsFilter): Promise<{ data: AnalyticsPeriodData | null; error: string | null }> {
  try {
    ...
    const currentPostBreakdown = calculatePostBreakdown(currentPosts);
    const previousPostBreakdown = calculatePostBreakdown(previousPosts);
...
export { mapLiveAnalyticsToPeriodData } from '../utils/analytics-transformer';
