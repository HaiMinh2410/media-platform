  ...
  const steps: PipelineStep[] = [
    ...
    },
    {
      ...
      howItWorks: [
        ...
        "Tự động tóm tắt cuộc hội thoại lũy tiến (Incremental Summarization): Nếu lịch sử trò chuyện vượt quá 50 tin nhắn và đã có bản tóm tắt cũ, hệ thống nhân bản an toàn dữ liệu lịch sử và tự động lọc bỏ trường tin nhắn thô 'lastMessages' trước khi gửi cho LLM. AI chỉ nhận bản tóm tắt cũ kèm các tin nhắn mới phát sinh để cập nhật lũy tiến, giúp tiết kiệm thêm hàng trăm token ngữ cảnh và tránh lỗi đột biến dữ liệu trong bộ nhớ.",
...
"{{incoming_message}}"`
    ...
    },
    {
      ...
      howItWorks: [
        ...
        "Chọn lọc Dynamic Few-shot: Chỉ nạp duy nhất 1 ví dụ few-shot tương ứng với chiến lược đang hoạt động (`strategy`) hoặc tự động hạ cấp về ví dụ `TrustBuilding` mặc định nếu không khớp. Điều này giúp tối ưu tiết kiệm ngay ~400 tokens cho mỗi lượt chat và giữ tập trung tuyệt đối cho mô hình.",
        "Bảo toàn cấu trúc JSON và Hỗ trợ Placeholders khi Override: Hỗ trợ tự động thay thế các placeholder '{{few_shot_examples}}' và '{{output_instructions}}' động bên trong prompt override (A/B Test hoặc Custom Prompt). Hệ thống tự động phát hiện để tránh nối lặp chỉ dẫn kỹ thuật JSON ở cuối prompt nếu các placeholder này đã được sử dụng.",
