...
export function RegisterForm() {
  ...
  return (
    <form className="flex flex-col gap-md" onSubmit={handleSubmit}>
      ...
      {error && (
        <div className="bg-status-error/10 border border-status-error text-status-error p-2xs rounded-sm text-sm text-center">
          {error}
        </div>
      )}
      ...
      <p className="text-center text-sm text-foreground-secondary mt-sm">
        ...
        <Link href="/auth/login" className="text-gradient font-semibold">
          Sign in
        </Link>
      </p>
    </form>
