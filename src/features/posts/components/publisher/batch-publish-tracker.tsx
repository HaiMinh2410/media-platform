    ...
    <div className="bg-base-100 border border-base-content/5 shadow-md rounded-3xl p-6 md:p-8 bg-gradient-to-br from-info/5 via-transparent to-transparent space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h3 className="text-xl font-brand font-bold text-base-content flex items-center gap-2">
            ...
            {status.status === 'RUNNING' && <Loader2 className="animate-spin text-info" size={18} />}
            {status.status === 'SCHEDULED' && <Calendar className="text-info" size={18} />}
          </h3>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm font-bold">
          ...
          <span className="text-base-content font-mono">{status.completed + status.failed} / {status.total}</span>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        <StatBox label="Hoàn thành" value={status.completed} color="text-success" icon={<CheckCircle2 size={14} />} />
        <StatBox label="Lỗi" value={status.failed} color="text-error" icon={<XCircle size={14} />} />
        <StatBox label="Đang chạy" value={status.running} color="text-info" icon={<Loader2 size={14} className="animate-spin" />} />
        <StatBox label="Đã lên lịch" value={status.scheduled || 0} color="text-info/80" icon={<Calendar size={14} />} />
        <StatBox label="Chờ" value={status.pending} color="text-base-content/50" />
      </div>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-1 gap-3">
            {status.jobs.filter(j => j.status === 'FAILED').map(job => (
              <div key={job.id} className="bg-error/5 border border-error/10 rounded-2xl p-4 flex items-start gap-4 transition-all hover:bg-error/10">
                ...
                <div className="space-y-1">
                  <div className="text-[15px] font-bold text-base-content flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 bg-base-200 border border-base-content/10 rounded-lg text-[10px] text-base-content/70 uppercase font-bold tracking-tight">
                      {job.platform}
                    </span>
                  </div>
...
function StatBox({ label, value, color, icon }: { label: string; value: number; color?: string; icon?: React.ReactNode }) {
  return (
    <div className="bg-base-200/50 rounded-2xl p-4 border border-base-content/5">
      ...
      <div className={cn("text-2xl font-bold font-mono", color || "text-base-content")}>{value}</div>
    </div>
  );
}
