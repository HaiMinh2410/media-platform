...
export function BatchPublishTracker({ batchId, onFinished }: BatchPublishTrackerProps) {
  ...
  if (error) {
    return (
      <div className="alert alert-error rounded-xl py-2 px-3 flex items-center gap-2 text-sm">
        <AlertCircle size={16} />
  ...
  return (
    <div className="w-full flex flex-col items-end">
      <AnimatePresence mode="wait">
        {isCollapsed ? (
          /* 1. Nút collapsed (chỉ hiển thị khi isCollapsed là true) */
          <motion.div
            key="collapsed"
          ...
          >
            <button
              ...
              className="btn btn-ghost btn-sm btn-circle hover:bg-base-200/80 cursor-pointer flex items-center justify-center relative"
              title={`Mở rộng theo dõi tiến độ đăng (${progress}%)`}
            >
              {status.status === "RUNNING" ? (
                <Loader2 className="animate-spin text-info" size={15} />
              ) : status.status === "SCHEDULED" ? (
                <Calendar className="text-info/70" size={15} />
              ) : (
                <CheckCircle2 className="text-success" size={15} />
              )}
              {status.status === "RUNNING" && (
                <span className="absolute -top-1 -right-1 bg-info text-info-content font-bold text-[8px] w-4 h-4 flex items-center justify-center rounded-full scale-90">
                  {progress}
                </span>
              )}
            </button>
          ...
          /* 2. Khối Expanded (trượt hiện ra khi isCollapsed là false) */
          <motion.div
            key="expanded"
            ...
            className="w-full"
          >
            <div className="py-4 relative flex flex-col gap-2.5">
              {/* Nút Thu gọn ở góc phải trên cùng */}
              <button
                onClick={() => setIsCollapsed(true)}
                className="absolute top-3 right-3 btn btn-soft btn-xs btn-circle text-base-content/40 hover:text-base-content hover:bg-base-200/80 cursor-pointer flex items-center justify-center transition-all z-10"
                title="Thu gọn theo dõi"
              >
                <ChevronUp size={14} />
              </button>

              {/* Giao diện đầy đủ (Expanded State) */}
              <div className="flex flex-col gap-2.5 pr-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 text-sm">
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-4 shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      {status.failed > 0 && isFinished && (
                        <button
                          onClick={handleRetry}
                          disabled={isRetrying}
                          className="btn btn-error btn-xs rounded-xl font-bold gap-1 py-1 px-2.5 h-7 min-h-0 cursor-pointer"
                        >
                          {isRetrying ? (
                    ...
                    <div className="text-2xs text-base-content/30 font-mono tracking-wider">
                      BATCH ID: <span className="font-mono text-base-content/50 font-medium normal-case select-all">{batchId}</span>
                    </div>
                  </div>
                </div>
                ...
                {status.failed > 0 && showErrors && (
                  <div className="pt-3 mt-1.5 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                    <h4 className="text-2xs font-bold text-base-content/40 uppercase tracking-widest font-mono flex items-center gap-1.5">
                          ...
                          >
                            ...
                            <div className="space-y-0.5">
                              <div className="text-xs font-bold text-base-content flex items-center gap-1.5">
                                ...
                                <span className="badge badge-sm badge-ghost uppercase font-bold text-3xs px-1 py-0.5 leading-none h-auto">
                                  {job.platform}
                                </span>
                              </div>
