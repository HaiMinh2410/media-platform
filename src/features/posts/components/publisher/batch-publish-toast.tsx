...
export function BatchPublishToast({
  initialAccounts,
  onClose,
  onRetry,
}: BatchPublishToastProps) {
  const [accounts, setAccounts] =
    useState<AccountPublishState[]>(initialAccounts);
    ...
    >
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {accounts.map((acc) => (
          <div
            key={acc.id}
            className="flex items-center gap-3 px-4 h-[52px] border-b border-base-content/5 last:border-0 text-base-content"
          >
            <div
              className="relative w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-[12px] shrink-0 overflow-hidden"
              style={{
                backgroundColor:
