  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragOver &&
          "border-sky-400 bg-[#f0f9ff]/80 dark:border-sky-800 dark:bg-base-250/70 scale-[1.01] shadow-2xs",
      )}
    >
      {/* Column Header */}
      <div className="flex justify-between items-center pb-2 border-b border-base-200 dark:border-base-800">
        <div className="flex items-center gap-1.5 text-xs md:text-sm font-bold text-base-content relative">
          ...
          <div className="relative group/tooltip flex items-center justify-center">
            <Info
              size={13}
              className="text-base-content/40 cursor-pointer hover:text-base-content/75 transition-colors shrink-0"
            />
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2.5 bg-white dark:bg-base-100 text-base-content/85 text-2xs leading-relaxed rounded-xl border border-base-200 dark:border-base-800 shadow-md opacity-0 pointer-events-none group-hover/tooltip:opacity-100 transition-opacity duration-300 z-50 text-center font-semibold">
          ...
          <span className="ml-1 w-5 h-5 rounded-full bg-base-200 dark:bg-base-800 text-2xs font-bold text-base-content/70 flex items-center justify-center font-mono">
            {leads.length}
          </span>
        </div>
        ...
        {leads.length > 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-5 text-center text-base-content/40 gap-4 mt-8">
            ...
            <div className="text-xs font-bold text-base-content/75 font-brand leading-snug">
              ...
              {stage.id !== "qualified" &&
                stage.id !== "converted" &&
                `Giai đoạn ${stage.label} trống`}
            </div>

            <p className="text-2xs leading-relaxed max-w-[200px] text-base-content/45 font-medium -mt-2">
              {stage.id === "qualified" &&
                "Chuyển khách hàng tiềm năng sang giai đoạn này nếu họ thực sự quan tâm đến sản phẩm hoặc dịch vụ."}
              {stage.id === "converted" &&
                "Chuyển khách hàng tiềm năng sang giai đoạn này nếu đã thỏa thuận hoặc giao dịch với họ."}
              {stage.id !== "qualified" &&
                stage.id !== "converted" &&
