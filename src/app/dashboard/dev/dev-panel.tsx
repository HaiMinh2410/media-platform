...
export function DevPanel({
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          toast.info(
            `🔔 Webhook raw mới nhận: Platform [${newRow.platform.toUpperCase()}]`,
          );
    ...
    try {
      ...
      const response = await fetch("/api/webhooks/meta", {
        ...
        headers: {
          ...
          "x-hub-signature-256":
            "sha256=mock_signature_from_dev_console_to_skip_verify_via_env",
        },
      ...
      if (response.ok) {
        ...
        toast.error(
          `Gửi webhook thất bại: ${response.status} - ${resBody.error || "Unknown"}`,
        );
      }
    } catch (error) {
      toast.dismiss();
  ...
  const handleClearDbLogs = async () => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn xóa TOÀN BỘ dữ liệu logs webhook trong cơ sở dữ liệu? Thao tác này không thể phục hồi.",
      )
    ) {
      return;
    }
      ...
      <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="glass p-5 rounded-lg flex flex-col gap-4">
            ...
            <div className="form-control w-full">
              <label className="label py-1">
                <span className="label-text text-foreground-secondary font-semibold text-xs">
                  Payload Mẫu
                </span>
              </label>
              <select
            ...
            {activePreset !== "custom" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="form-control w-full">
                  <label className="label py-1">
                    <span className="label-text text-foreground-secondary text-xs">
                ...
                <div className="form-control w-full">
                  <label className="label py-1">
                    <span className="label-text text-foreground-secondary text-xs flex justify-between">
                      ...
                      <span className="text-accent-primary text-3xs font-semibold">
                        Khớp Account
                      </span>
                    </span>
                  </label>
                    ...
                    >
                      {connectedAccounts.map((acc) => (
                        <option key={acc.id} value={acc.platform_user_id}>
                          ...
                          {acc.platform_user_name} ({acc.platform_user_id})
                        </option>
                      ))}
            ...
            <div className="form-control w-full flex-1">
              <label className="label py-1 flex justify-between">
                ...
                {activePreset === "custom" && (
                  <span className="text-3xs text-foreground-tertiary">
        ...
        <div className="glass p-5 rounded-lg flex flex-col gap-3">
          <div className="flex justify-between items-center">
            <h4 className="text-sm font-extrabold tracking-wide uppercase text-foreground-secondary">
              Database Statistics
            </h4>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-1">
            <div className="bg-foreground/5 border border-foreground/5 p-3 rounded-lg text-center">
              <div className="text-foreground-secondary text-4xs font-bold uppercase">
                Raw Logs
              </div>
              <div className="text-lg font-mono font-extrabold text-foreground mt-0.5">
                {stats?.rawLogs ?? 0}
              </div>
            </div>
            <div className="bg-foreground/5 border border-foreground/5 p-3 rounded-lg text-center">
        ...
        <div className="glass p-5 rounded-lg flex-1 flex flex-col min-h-[550px] max-h-[850px]">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`w-2.5 h-2.5 rounded-full inline-block ${isSubscribed ? "bg-success animate-pulse" : "bg-red-500"}`}
                title={
            ...
            {streamItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 text-foreground-secondary gap-2">
                ...
                <div className="font-semibold text-sm">
                  Chưa có sự kiện webhook nào trong phiên này
                </div>
                <div className="text-xs text-foreground-tertiary max-w-xs">
                  Gửi webhook giả lập bên trái hoặc thao tác thật trên
                  Facebook/Instagram để xem dữ liệu cập nhật realtime tại đây.
                </div>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex justify-between items-center text-3xs font-mono mb-1">
                        ...
                        <div className="flex gap-1">
                          <span
                            className={`px-1.5 py-0.5 rounded uppercase font-extrabold ${
                      ...
                      {log.webhookEvents && log.webhookEvents.length > 0 && (
                        <div className="mt-1.5 border-t border-foreground/5 pt-1 flex flex-col gap-0.5 text-2xs font-mono text-foreground-secondary">
                          {log.webhookEvents.map((ev, i) => (
                            ...
                            >
                              <span className="text-foreground-tertiary truncate max-w-[150px]">
            ...
            <div className="border-t border-foreground/10 pt-4 mt-4 flex flex-col gap-3 h-1/2 overflow-y-auto">
              <div className="flex justify-between items-center">
                <span className="text-xs font-extrabold text-foreground flex items-center gap-2">
                  🔍 Chi tiết Event Log
                  <span className="font-mono text-3xs bg-foreground/10 px-1 rounded">
              ...
              <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
                ...
                selectedItem.associations.length > 0 ? (
                  <div className="bg-success/5 border border-success/20 rounded-md p-3 text-2xs font-mono flex flex-col gap-2">
                    <div className="font-bold text-success text-3xs uppercase tracking-wider">
                    ...
                    {selectedItem.associations.map((assoc, idx) => (
                      <div
                        key={idx}
                        className="border-t border-success/10 pt-2 first:border-0 first:pt-0"
                      >
                        ...
                        {assoc.conversation ? (
                          <div className="mt-1 bg-black/30 p-2 rounded flex flex-col gap-1">
                            ...
                            <div className="flex justify-between text-foreground-secondary">
                              <span>Status:</span>
                              <span className="badge badge-3xs badge-outline uppercase font-extrabold">
                              ...
                              <div className="flex flex-col gap-1 mt-1 max-h-24 overflow-y-auto">
                                {assoc.conversation.messages
                                  ...
                                  .map((msg) => (
                                    <div
                                      key={msg.id}
                                      className="flex justify-between bg-foreground/2 p-1 rounded gap-1 text-2xs"
                                    >
                                      <span
                                        className={
                                          msg.senderType === "user"
