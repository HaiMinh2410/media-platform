    ...
    return (
      <div className="p-10 max-w-[1400px] mx-auto">
        <header className="mb-10">
          <h1 className="text-40 mb-1 font-brand font-bold text-gradient">Dashboard</h1>
        </header>
  ...
  return (
    <div className="p-10 max-w-[1400px] mx-auto">
      <header className="mb-10 flex justify-between items-start">
        <div>
          <h1 className="text-40 mb-1 font-brand font-bold text-gradient">Dashboard</h1>
          <p className="text-foreground-secondary text-lg">Welcome back! Here's what's happening with your accounts today.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-6 mb-10">
        <div className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-2 transition-all hover:-translate-y-1 hover:border-primary/50">
          <span className="text-sm text-foreground-secondary font-medium uppercase tracking-wider">Connected Accounts</span>
          <span className="text-32 font-bold font-brand">{stats.accounts}</span>
          <span className="text-sm flex items-center gap-1 text-foreground-tertiary">
            Accounts linked
          </span>
        </div>
        <div className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-2 transition-all hover:-translate-y-1 hover:border-primary/50">
          <span className="text-sm text-foreground-secondary font-medium uppercase tracking-wider">Total Messages</span>
          <span className="text-32 font-bold font-brand">{stats.messages}</span>
          <span className="text-sm flex items-center gap-1 text-foreground-tertiary">
            Interactions recorded
          </span>
        </div>
        <div className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-2 transition-all hover:-translate-y-1 hover:border-primary/50">
          <span className="text-sm text-foreground-secondary font-medium uppercase tracking-wider">Conversations</span>
          <span className="text-32 font-bold font-brand">{stats.conversations}</span>
          <span className="text-sm flex items-center gap-1 text-foreground-tertiary">
            Total active threads
          </span>
        </div>
        <div className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-2 transition-all hover:-translate-y-1 hover:border-primary/50">
          <span className="text-sm text-foreground-secondary font-medium uppercase tracking-wider">Webhook Events</span>
          <span className="text-32 font-bold font-brand">{stats.events}</span>
          <span className="text-sm flex items-center gap-1 text-foreground-tertiary">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
        <section className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-5">
          <div className="text-xl font-semibold flex justify-between items-center text-foreground">
            Recent Messages
            <Link href="/dashboard/inbox" className="text-gradient text-sm">View All</Link>
          </div>
          <div className="flex flex-col gap-3">
            {recentMessages.length > 0 ? (
                ...
                return (
                  <div key={msg.id} className="p-4 rounded-xl bg-foreground/[0.02] border border-foreground/5 flex items-center gap-4 transition-all hover:bg-foreground/[0.05]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-md text-foreground truncate">{conversation.customer_name || msg.senderId}</div>
                      <div className="text-13 text-foreground-secondary truncate">{msg.content.substring(0, 60)}{msg.content.length > 60 ? '...' : ''}</div>
                    </div>
                    <div className="text-13 text-foreground-tertiary shrink-0">
              ...
              <div className="text-13 text-foreground-tertiary p-5 text-center">No messages yet.</div>
            )}
          </div>
        </section>
        ...
        <section className="glass p-6 rounded-2xl border border-foreground/10 bg-foreground/[0.02] backdrop-blur-xl flex flex-col gap-5">
          <div className="text-xl font-semibold flex justify-between items-center text-foreground">
            Your Accounts
            <Link href="/dashboard/settings/accounts" className="text-gradient text-sm">Manage</Link>
          </div>
          <div className="flex flex-col gap-3">
            {activeAccounts.length > 0 ? (
                ...
                <div key={acc.id} className="p-4 rounded-xl bg-foreground/[0.02] border border-foreground/5 flex items-center gap-4 transition-all hover:bg-foreground/[0.05]">
                  <div 
                    className={cn(
                      ...
                      acc.platform === 'facebook' ? "bg-facebook" : 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-md text-foreground truncate">{acc.platform_user_name}</div>
                    <div className="text-13 text-foreground-secondary capitalize">{acc.platform}</div>
                  </div>
              ...
              <div className="text-13 text-foreground-tertiary p-5 text-center">No accounts connected.</div>
            )}
          </div>
        </section>
      </div>
